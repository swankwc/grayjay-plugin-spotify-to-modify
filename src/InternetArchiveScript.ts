...
const PLATFORM = "Internet Archive" as const
...
const local_source: InternetArchiveSource = {
    enable,
    disable,
    saveState,
    getHome,
...
function init_source(local_source: Source<any, any, any, any, any, any>) {
...
//#region enable
function enable(_conf: SourceConfig, _settings: Settings, _savedState?: string | null) {
}
...
function getSearchCapabilities() {
    return new ResultCapabilities<string, typeof Type.Feed.Mixed>(
        [Type.Feed.Mixed],
        [Type.Order.Chronological, Type.Order.Views],
        []
    )
}

function search(query: string, _type: string | null, order: Order | null, _filters: FilterQuery<string> | null): VideoPager {
...
class IASearchPager extends VideoPager {
    ...
    override nextPage(this: IASearchPager): IASearchPager {
        this.page++
    ...
    override hasMorePagers(this: IASearchPager): boolean {
        return this.hasMore
    }
}

function fetchSearch(query: string, page: number, limit: number, order: Order | null): IASearchResponse {
...
function formatSearchDocs(docs: IADoc[]): PlatformVideo[] {
    return docs.map(doc => {
        ...
        return new PlatformVideo({
            ...
            author: new PlatformAuthorLink(
                new PlatformID(PLATFORM, author_id || "unknown", plugin.config.id),
                author_name || "Unknown",
                `https://archive.org/details/${author_id}`
            ),
...
function getContentDetails(url: string) {
    ...
    if (!match || !match[1]) throw new ScriptException("Invalid IA URL")
    const identifier = match[1]

    const response = local_http.GET(`${METADATA_URL_PREFIX}${identifier}`, { "User-Agent": USER_AGENT }, false)
    if (!response.isOk) throw new ScriptException("Failed to fetch IA metadata")
    const metadata: IAMetadataResponse = JSON.parse(response.body)

    const video_files = metadata.files.filter(f =>
        f.format === "MPEG4" || f.format === "h.264" || f.format === "WebM" || f.format === "Ogg Video"
    )

    if (video_files.length === 0) throw new ScriptException("No playable video files found")

    // Sort to prefer MPEG4 and then by size/length
    video_files.sort((a, b) => {
        if (a.format === "MPEG4" && b.format !== "MPEG4") return -1
        if (a.format !== "MPEG4" && b.format === "MPEG4") return 1
    ...
    const video_sources = video_files.map(f => {
        ...
        const source_def: IVideoUrlSourceDef = {
...
function getChannelContents(url: string, type: string | null, order: Order | null, filters: FilterQuery<string> | null) {
    const match = url.match(DETAILS_REGEX)
    if (!match || !match[1]) throw new ScriptException("Invalid IA URL")
    const identifier = match[1]
...
function searchChannels(query: string): ChannelPager {
    ...
    const response = local_http.GET(url.toString(), { "User-Agent": USER_AGENT }, false)
    if (!response.isOk) throw new ScriptException("Channel search failed")
    const search_response: IASearchResponse = JSON.parse(response.body)
...
class IAChannelPager extends ChannelPager {
    constructor(results: PlatformChannel[], hasMore: boolean, private query: string, private page: number) {
        super(results, hasMore)
    }

    override nextPage(this: IAChannelPager): IAChannelPager {
        this.page++
        const url = new URL(SEARCH_URL)
